...
export default function ProgressTimeline({ currentStep }) {
  ...
  return (
    <div className="stripe-card p-6 border border-slate-800 bg-[#0f172a]/90 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400">Automated reasoning and policy validation in progress...</p>
        </div>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-[10px] font-mono font-medium text-slate-500">0{step.id}</span>
              </div>
              <p className={`text-xs font-semibold ${isCurrent ? 'text-blue-300' : isDone ? 'text-emerald-300' : 'text-slate-400'}`}>
                {step.label}
              </p>
