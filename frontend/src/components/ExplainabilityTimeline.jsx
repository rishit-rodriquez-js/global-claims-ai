    ...
    <div className="stripe-card p-6 border border-slate-800 bg-[#0f172a] space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400">Click any phase to inspect evidence and reasoning logs</p>
        </div>
        <span className="text-[11px] text-slate-400 bg-slate-800 px-2 py-1 rounded font-mono border border-slate-700">
        ...
        {stages.map((st, index) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[11px] font-semibold leading-tight">{st.step}</span>
            </button>
          );
        })}
      </div>
      ...
      <div className="p-4 rounded-lg bg-slate-900/90 border border-slate-800 flex items-start gap-3">
        ...
        <div className="space-y-1 text-left">
          <div className="flex items-center justify-between">
            <h4 className="text-xs font-semibold text-white">{currentActive.title}</h4>
            {currentActive.timestamp && (
              <span className="text-[10px] text-slate-500 font-mono">{currentActive.timestamp}</span>
            )}
          </div>
          <p className="text-xs text-slate-300 leading-relaxed">{currentActive.detail}</p>
        </div>
      </div>
    </div>
  );
