...
class ClaimModel(Base):
    ...
    status = Column(String, default="NEW")  # NEW, HUMAN_REVIEW, APPROVED, REJECTED, MORE_INFO_REQUIRED
    ...
    rag_time_ms = Column(Float, default=0.0)
    llm_time_ms = Column(Float, default=0.0)
    total_pipeline_time_ms = Column(Float, default=0.0)
    created_at = Column(String, default=lambda: datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
...
class DocumentModel(Base):
    ...
    stored_blob_name = Column(String, nullable=True)
    blob_url = Column(String, nullable=True)
    container_name = Column(String, default="claims-documents")
    ...
    azure_etag = Column(String, nullable=True)
    upload_time = Column(String, default=lambda: datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
